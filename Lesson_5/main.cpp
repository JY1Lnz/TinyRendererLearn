...
m4f lockAt(Vec3f eye, Vec3f center, Vec3f up)
{
    // 模型变换，将一个物体变换到相机空间
...
{
	// 这里是视口变换，把[-1,1]^3 的空间变换到[x, x+w][y, y+h][0, depth]的位置
	// 貌似和有远近平面的不太一样
	// 对于x，有x = -1, x' = x; x = 1, x' = x+w,解个方程就ok了
...
int main()
{
	...
	Vec3f eye = Vec3f(1, 0, 1);
	Vec3f center = Vec3f(0, 0, 0);

	m4f view_mat = lockAt(eye, center, Vec3f(0, 1, 0));
	m4f view_prot = viewport(0, 0, width, height);

	Vec3f eye_dir = (center - eye).normalize();
	...
	for (int i = 0; i < model->nfaces(); ++i)
	{
		...
		for (int j = 0; j < 3; ++j)
		{
			...
			world_coords[j] = p;
			proj_coords[j] = m2v((proj * view_mat * v2m(p)));
			screen_coords[j] = m2v(view_prot * v2m(proj_coords[j]));
		}
		Vec3f normal = (world_coords[2] - world_coords[0]) ^
			(world_coords[1] - world_coords[0]);
		normal.normalize();
		float intensity = normal * light_dir;
		float culling = normal * eye_dir;
		if (culling > 0)
